...
class Basecamp
{
  ...
  /**
   * The base URL.
   * example: http://mycompany.basecamphq.com/
   *
   * @var string
   */
  protected $baseurl;
   ...
   * @param  null|string  $extended_body
   * @param  bool         $private
   * @param  array        $notify_people
  ...
  {
    $resource_type = strtolower($resource_type);
    if (!in_array($resource_type, array('me', 'person', 'company', 'unassigned')))
    {
      throw new InvalidArgumentException("'{$resource_type}' is an invalid resource type.");
    }
  ...
  public function getMilestonesForProject($project_id, $filter_type = 'all', $format = null)
  {
    ...
    $this->setupRequestBody(array('find' => $filter_type));
    ...
    $filter_company_id = null)
  {
    ...
    return $this->processRequest(
      sprintf(
        "{$this->baseurl}time_entries/report.xml?from=%s&to=%s&subject_id=%s&to_item_id=%s&filter_project_id=%s&filter_company_id=%s",
         $from, $to, (int) $subject_id, (int) $to_item_id, (int) $filter_project_id, (int) $filter_company_id
      ), "GET"
    );
  }
  ...
  private function createXMLFromArray($array, $level = 0)
  {
    ...
    foreach ($array as $key => $val) {
      ...
      $xml .= sprintf("%s<%s>%s</%s>\n",
                      ...
                      is_array($val) ? "\n" . $this->createXMLFromArray($val, $level + 1) . str_repeat('  ', $level)
