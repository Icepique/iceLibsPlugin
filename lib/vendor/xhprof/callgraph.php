...
function  xhprof_render_diff_image($xhprof_runs_impl, $run1, $run2,
  ...
  list($raw_data1, $a) = $xhprof_runs_impl->get_run($run1, $source, null);
  list($raw_data2, $b) = $xhprof_runs_impl->get_run($run2, $source, null);
  ...
  $symbol_tab1 = xhprof_compute_flat_info($raw_data1, $dummy);
  $symbol_tab2 = xhprof_compute_flat_info($raw_data2, $dummy);
...
function xhprof_get_content_by_run($xhprof_runs_impl, $run_id, $type,
  ...
  list($raw_data, $a) = $xhprof_runs_impl->get_run($run_id, $source, $dummy);
  if (!$raw_data) {
