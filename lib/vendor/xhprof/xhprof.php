...
include_once XHPROF_LIB_ROOT . '/xhprof_lib.php';
include_once XHPROF_LIB_ROOT . '/callgraph.php';
...
function xhprof_include_js_css($ui_dir_url_path = null) {
  ...
  // javascript
  echo "<script src='$ui_dir_url_path/jquery/jquery.js'>".
       "</script>";
...
function print_flat_data($url_params, $title, $flat_data, $sort, $run1, $run2, $limit) {
  ...
  include dirname(__FILE__) ."/templates/profChart.phtml";
  include dirname(__FILE__) ."/templates/profTable.phtml";
...
function full_report($url_params, $symbol_tab, $sort, $run1, $run2, $links) {
  ...
  if ($diff_mode) {
      ...
      include dirname(__FILE__) ."/templates/diff_run_header_block.phtml";

  } else {
      global $xhprof_runs_impl;
    include dirname(__FILE__) ."/templates/single_run_header_block.phtml";
  }
