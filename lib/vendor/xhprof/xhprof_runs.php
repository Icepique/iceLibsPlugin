...
class XHProfRuns_Default implements iXHProfRuns
{
  ...
  public function getRuns($stats)
  {
    if (isset($stats['select'])) {
      $query = "SELECT {$stats['select']} FROM `xhprof` ";
    }
    else
    {
      $query = "SELECT * FROM `xhprof` ";
    }
  ...
  public function getDistinct($data)
  {
    ...
    $query = "SELECT DISTINCT(`{$sql['column']}`) FROM `xhprof`";
  ...
  public function get_run($run_id, $type, &$run_desc)
  {
    ...
    $query = "SELECT *, UNCOMPRESS(`perfdata`) AS `perfdata` FROM `xhprof` WHERE `id` = '".$run_id."'";
  ...
  public function getRunComparativeData($url, $c_url)
  {
    ...
    $query = "SELECT count(`id`), avg(`wt`), min(`wt`), max(`wt`),  avg(`cpu`), min(`cpu`), max(`cpu`), avg(`pmu`), min(`pmu`), max(`pmu`) FROM `xhprof` WHERE `url` = '$url'";
    ...
    //Runs same c_url
    //  count, avg/min/max for wt, cpu, pmu
    $query = "SELECT count(`id`), avg(`wt`), min(`wt`), max(`wt`),  avg(`cpu`), min(`cpu`), max(`cpu`), avg(`pmu`), min(`pmu`), max(`pmu`) FROM `xhprof` WHERE `c_url` = '$c_url'";
    $rs = mysql_query($query, $this->linkID);
    $row = mysql_fetch_assoc($rs);
    $row['url'] = $c_url;
  ...
  protected function calculatePercentile($details)
  {
    ...
    $query = "SELECT `{$details['column']}` as `value` FROM `xhprof` WHERE `{$details['type']}` = '{$details['url']}' ORDER BY `{$details['column']}` DESC LIMIT $limit, 1";
    $rs = mysql_query($query, $this->linkID);
    $row = mysql_fetch_assoc($rs);
    return $row['value'];
  }
  ...
  public function save_run($xhprof_data, $type, $run_id = null, $xhprof_details = null)
  {
    ...
    $sql['url'] = mysql_real_escape_string(urldecode($url));
    $sql['c_url'] = mysql_real_escape_string(urldecode(_urlSimilartor($_SERVER['REQUEST_URI'])));
